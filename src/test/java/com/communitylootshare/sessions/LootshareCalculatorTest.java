...
public class LootshareCalculatorTest
{
	...
	@Test
	public void routesEverySettlementTransferThroughTheHost()
	{
		LootshareSession session = new LootshareSession("host-routes", 10L, Instant.EPOCH);
		session.setHostState(1L, 0L, 1L);
		session.addAcceptedProposal(accepted("first", 2L, 90L, Instant.ofEpochSecond(1),
			new LootshareParticipant(1L, "Alice"), new LootshareParticipant(2L, "Bob"),
			new LootshareParticipant(3L, "Charlie")));
		session.addAcceptedProposal(accepted("second", 1L, 60L, Instant.ofEpochSecond(2),
			new LootshareParticipant(1L, "Alice"), new LootshareParticipant(2L, "Bob"),
			new LootshareParticipant(3L, "Charlie")));
		...
		assertEquals(2, result.getTransfers().size());
		assertTransfer(result.getTransfers().get(0), 2L, "Bob", 1L, "Alice", 40L);
		assertTransfer(result.getTransfers().get(1), 1L, "Alice", 3L, "Charlie", 50L);
	}
