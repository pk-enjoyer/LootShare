...
public class LootshareControllerTest
{
	...
	@Test
	public void ignoresLootKeysAndPlayerLoot()
	{
		LootReceived lootKey = mock(LootReceived.class);
		when(lootKey.getType()).thenReturn(LootRecordType.EVENT);
		when(lootKey.getName()).thenReturn("Loot keys");
		when(lootKey.getItems()).thenReturn(Collections.singletonList(new ItemStack(100, 1)));
		...
		when(playerLoot.getItems()).thenReturn(Collections.singletonList(new ItemStack(101, 1)));

		controller.onLootReceived(lootKey);
	...
	@Test
	public void electsTheFirstMemberAndAppliesRemoteHostConfiguration()
	{
		...
		assertEquals(MutationResult.APPLIED, engine.updateHostState(localMember.getMemberId(),
			remoteMember.getMemberId(), engine.getActiveHostSettings().orElse(null), 2L));
	...
	@Test
	public void guestUsesTheHostCapturePolicy()
	{
		assertEquals(MutationResult.APPLIED, engine.updateHostState(localMember.getMemberId(),
			remoteMember.getMemberId(), engine.getActiveHostSettings().orElse(null), 2L));
		...
		when(captureService.capture(eq("Raid reward"), eq(stacks), eq("Alice"), any(Instant.class), eq(42L)))
			.thenReturn(Optional.of(event("host-event", "Alice", 300L)));
	...
	public void hostPolicyCanIncludeLoggedOutMembersInTheFrozenRoster()
	{
		...
		when(received.getItems()).thenReturn(stacks);
		when(captureService.capture(eq("Boss"), eq(stacks), eq("Alice"), any(Instant.class), eq(42L)))
			.thenReturn(Optional.of(event("offline", "Alice", 200L)));
	...
	@Test
	public void excludesNeverResolvedLoggedOutMembersButIncludesThemAfterIdentitySync()
	{
		remoteMember.setDisplayName("<unknown>");
		remoteMember.setLoggedIn(false);
		when(config.includeLoggedOutMembers()).thenReturn(true);
		controller.onLocalConfigurationChanged();
		assertEquals(MutationResult.APPLIED,
		...
		List<ItemStack> stacks = Collections.singletonList(new ItemStack(100, 1));
		LootReceived first = mock(LootReceived.class);
		when(first.getType()).thenReturn(LootRecordType.NPC);
		when(first.getName()).thenReturn("Boss");
		when(first.getItems()).thenReturn(stacks);
		when(captureService.capture(eq("Boss"), eq(stacks), eq("Alice"), any(Instant.class), eq(42L)))
			.thenReturn(Optional.of(event("unknown-offline", "Alice", 200L)),
				Optional.of(event("known-offline", "Alice", 300L)));

		controller.onLootReceived(first);
		assertEquals(1, engine.getProposal("unknown-offline").get().getParticipants().size());

		MemberIdentityMessage identity = new MemberIdentityMessage("Bob");
		identity.setMemberId(remoteMember.getMemberId());
		controller.onMemberIdentityMessage(identity);
		assertEquals("Bob", controller.getKnownMemberName(remoteMember).get());

		LootReceived second = mock(LootReceived.class);
		when(second.getType()).thenReturn(LootRecordType.NPC);
		when(second.getName()).thenReturn("Boss");
		when(second.getItems()).thenReturn(stacks);
		controller.onLootReceived(second);
		assertEquals(2, engine.getProposal("known-offline").get().getParticipants().size());
	}
