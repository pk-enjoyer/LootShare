...
public class UiControllerTest
{
	...
	public void setUp()
	{
		...
		controller = new UiController(client, clientThread, partyService, itemManager, lootshareController, partyConfig);
	}
	...
	@Test
	public void labelsNeverResolvedMembersWithoutExposingTheirPartyId()
	{
		PartyMember remote = member(2L, "<unknown>", false);
		when(partyService.isInParty()).thenReturn(true);
		when(partyService.getMembers()).thenReturn(Collections.singletonList(remote));
		when(lootshareController.isReady()).thenReturn(true);
		when(lootshareController.getActivePartyProposals()).thenReturn(Collections.emptyList());

		PanelState state = controller.buildState();

		assertEquals("Name unavailable", state.getMembers().get(0).getDisplayName());
	}
