...
public class CommunityLootsharePanel extends PluginPanel
{
	...
	private JPanel buildSettlementSection()
	{
		...
		settlementGraphsButton.setFocusable(false);
		settlementGraphsButton.setToolTipText("Open settlement graphs in a separate window");
		settlementGraphsButton.addActionListener(event -> actions.openSettlementDashboard());
		header.add(settlementToggle, BorderLayout.CENTER);
		header.add(settlementGraphsButton, BorderLayout.EAST);
	...
	private void renderHostedSettings(HostedSettings hostedSettings)
	{
		...
		else
		{
			...
			hostedSettingsBody.add(settingRow("New loot value", settings.getLootValueBasis().getDisplayName()));
			...
			hostedSettingsBody.add(settingRow("Player/PvP loot", included(settings.isCapturePlayerLoot())));
			hostedSettingsBody.add(settingRow("Pickpocket loot", included(settings.isCapturePickpocketLoot())));
	...
	private void renderSettlement(CommunityLootsharePanelState state)
	{
		...
		settlementView.setVisible(settlementExpanded);
		settlementGraphsButton.setEnabled(state.isReady());
		updateSettlementToggleText();
	}
	...
	JButton getSettlementGraphsButton()
	{
	...
	final class MemberCard extends JPanel
	{
		...
		private final JMenuItem transferHostItem;
		...
		private MemberCard(MemberLoot member, boolean expanded)
		{
			...
			transferHostItem = buildTransferHostItem(member);
			...
			String tooltip = "Show or hide " + member.getDisplayName() + "'s drops";
			if (transferHostItem != null)
			{
				tooltip += "; right-click to transfer host";
			}
			installHeaderInteraction(header, toggleListener, tooltip);
			if (transferHostItem != null)
			{
				JPopupMenu popupMenu = new JPopupMenu();
				popupMenu.add(transferHostItem);
				installPopupMenu(header, popupMenu);
			}
		...
		private void installPopupMenu(Component component, JPopupMenu popupMenu)
		{
			if (component instanceof AbstractButton)
			{
				return;
			...
			if (component instanceof Container)
			{
				for (Component child : ((Container) component).getComponents())
				{
					installPopupMenu(child, popupMenu);
				}
			}
		}
		...
		private JMenuItem buildTransferHostItem(MemberLoot member)
		{
			...
			JMenuItem item = new JMenuItem("Transfer host");
			item.addActionListener(event -> requestHostTransfer(member));
			return item;
		}
		...
		private JPanel buildMemberHeader(MemberLoot member)
		{
			...
			if (localHost && !member.isHost())
			{
				if (transferHostItem != null)
				{
					transferHostButton = new JButton("Make host");
					transferHostButton.setFont(FontManager.getRunescapeSmallFont());
					transferHostButton.setFocusable(false);
					transferHostButton.setMargin(new Insets(1, 4, 1, 4));
					transferHostButton.setAlignmentX(RIGHT_ALIGNMENT);
					transferHostButton.addActionListener(event -> requestHostTransfer(member));
					status.add(transferHostButton);
				}
		...
		JMenuItem getTransferHostItem()
		{
			return transferHostItem;
		}

		JButton getTransferHostButton()
		{
			return transferHostButton;
		}
