...
public final class LootshareSession
{
	...
	public LootshareSession(long partyId)
	{
		if (partyId <= 0L)
		{
	...
	public void setHost(long id, LootshareSettings next, long revision)
	{
		hostMemberId = id;
		settings = next;
	...
	{
		List<LootProposal> result = new ArrayList<>();
		for (LootProposal p : proposals)
		{
			if (p.isAccepted())
			{
				result.add(p);
	...
	public LootProposal find(String id)
	{
		for (LootProposal p : proposals)
		{
	...
	public boolean add(LootProposal proposal)
	{
		if (find(proposal.getProposalId()) != null)
		{
			return false;
		}
