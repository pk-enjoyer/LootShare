...
public class DecisionMessage extends PartyMemberMessage
{
	...
	{
		if (!proposal.isAccepted() || proposal.getDecidedAt() == null)
		{
			throw new IllegalArgumentException("Accepted proposal required");
		}
		proposalId = proposal.getProposalId();
		decidedAtEpochMilli = proposal.getDecidedAt().toEpochMilli();
		for (LootshareParticipant participant : proposal.getParticipants())
		{
			participants.add(new ParticipantPayload(participant));
		}
	}
	...
	public Optional<DecodedDecision> decode()
	{
		if (protocolVersion != PROTOCOL_VERSION || getMemberId() <= 0L || proposalId == null || participants == null
			|| participants.isEmpty() || participants.size() > LootProposal.MAX_PARTICIPANTS)
		{
	...
	public static class ParticipantPayload
	{
		...
		private ParticipantPayload(LootshareParticipant participant)
		{
